                               ...
                               struct match *match)
{
    ...
    if (!(ofpfw & OFPFW10_DL_VLAN) &&
        ...
        match->flow.ivlan.vlan_tci = htons(0);
        ...
        match->flow.ivlan.vlan_tci = tci & match->wc.masks.ivlan.vlan_tci;
    }
                             ...
                             struct ofp10_match *ofmatch)
{
    ...
    if (match->wc.masks.ivlan.vlan_tci == htons(0)) {
        ofpfw |= OFPFW10_DL_VLAN | OFPFW10_DL_VLAN_PCP;
    } else if (match->wc.masks.ivlan.vlan_tci & htons(VLAN_CFI)
               && !(match->flow.ivlan.vlan_tci & htons(VLAN_CFI))) {
        ofmatch->dl_vlan = htons(OFP10_VLAN_NONE);
    } else {
        if (!(match->wc.masks.ivlan.vlan_tci & htons(VLAN_VID_MASK))) {
            ofpfw |= OFPFW10_DL_VLAN;
        } else {
            ofmatch->dl_vlan = htons(vlan_tci_to_vid(match->flow.ivlan.vlan_tci));
        }

        if (!(match->wc.masks.ivlan.vlan_tci & htons(VLAN_PCP_MASK))) {
            ofpfw |= OFPFW10_DL_VLAN_PCP;
        } else {
            ofmatch->dl_vlan_pcp = vlan_tci_to_pcp(match->flow.ivlan.vlan_tci);
        }
    }
    ...
    if (!(wc & OFPFW11_DL_VLAN)) {
        if (ofmatch->dl_vlan == htons(OFPVID11_NONE)) {
            ...
            if (ofmatch->dl_vlan == htons(OFPVID11_ANY)) {
                ...
                match->flow.ivlan.vlan_tci = htons(VLAN_CFI);
                match->wc.masks.ivlan.vlan_tci = htons(VLAN_CFI);
            } else if (ntohs(ofmatch->dl_vlan) < 4096) {
                /* Match only packets with the specified VLAN VID. */
                match->flow.ivlan.vlan_tci = htons(VLAN_CFI) | ofmatch->dl_vlan;
                match->wc.masks.ivlan.vlan_tci = htons(VLAN_CFI | VLAN_VID_MASK);
            } else {
                /* Invalid VID. */
                return OFPERR_OFPBMC_BAD_VALUE;
            }
            ...
            if (!(wc & OFPFW11_DL_VLAN_PCP)) {
                if (ofmatch->dl_vlan_pcp <= 7) {
                    match->flow.ivlan.vlan_tci |= htons(ofmatch->dl_vlan_pcp
                                                  << VLAN_PCP_SHIFT);
