           ...
           ovs_be64 cookie, ovs_be64 cookie_mask)
{
    ...
    if (oxm) {
        ...
        ovs_be16 vid = flow->ivlan.vlan_tci & VID_CFI_MASK;
        ovs_be16 mask = match->wc.masks.ivlan.vlan_tci & VID_CFI_MASK;
        ...
        if (vid && vlan_tci_to_pcp(match->wc.masks.ivlan.vlan_tci)) {
            nxm_put_8(b, MFF_VLAN_PCP, oxm,
                      vlan_tci_to_pcp(flow->ivlan.vlan_tci));
        }

    } else {
        nxm_put_16m(b, MFF_VLAN_TCI, oxm, flow->ivlan.vlan_tci,
                    match->wc.masks.ivlan.vlan_tci);
    }
