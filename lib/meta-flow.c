...
mf_is_all_wild(const struct mf_field *mf, const struct flow_wildcards *wc)
{
    ...
    case MFF_VLAN_TCI:
        return !wc->masks.ivlan.vlan_tci;
    case MFF_DL_VLAN:
        return !(wc->masks.ivlan.vlan_tci & htons(VLAN_VID_MASK));
    case MFF_VLAN_VID:
        return !(wc->masks.ivlan.vlan_tci & htons(VLAN_VID_MASK | VLAN_CFI));
    case MFF_DL_VLAN_PCP:
    case MFF_VLAN_PCP:
        return !(wc->masks.ivlan.vlan_tci & htons(VLAN_PCP_MASK));
...
mf_are_prereqs_ok(const struct mf_field *mf, const struct flow *flow)
{
    ...
    case MFP_VLAN_VID:
        return (flow->ivlan.vlan_tci & htons(VLAN_CFI)) != 0;
    case MFP_MPLS:
...
mf_mask_field_and_prereqs(const struct mf_field *mf, struct flow_wildcards *wc)
{
    ...
    case MFP_IP_ANY:
        ...
        break;
    case MFP_VLAN_VID:
        WC_MASK_FIELD_MASK(wc, ivlan.vlan_tci, htons(VLAN_CFI));
        break;
    case MFP_NONE:
        break;
    }
             ...
             union mf_value *value)
{
    ...
    case MFF_VLAN_TCI:
        value->be16 = flow->ivlan.vlan_tci;
        break;

    case MFF_DL_VLAN:
        value->be16 = flow->ivlan.vlan_tci & htons(VLAN_VID_MASK);
        break;
    case MFF_VLAN_VID:
        value->be16 = flow->ivlan.vlan_tci & htons(VLAN_VID_MASK | VLAN_CFI);
        break;

    case MFF_DL_VLAN_PCP:
    case MFF_VLAN_PCP:
        value->u8 = vlan_tci_to_pcp(flow->ivlan.vlan_tci);
        break;
                  ...
                  const union mf_value *value, struct flow *flow)
{
    ...
    case MFF_VLAN_TCI:
        flow->ivlan.vlan_tci = value->be16;
        break;
