...
ofctl_check_vlan(struct ovs_cmdl_context *ctx)
{
    ...
    match.flow.ivlan.vlan_tci = htons(strtoul(ctx->argv[1], NULL, 16));
    match.wc.masks.ivlan.vlan_tci = htons(strtoul(ctx->argv[2], NULL, 16));
    ...
    printf("%04"PRIx16"/%04"PRIx16"\n",
           ntohs(fm.match.flow.ivlan.vlan_tci),
           ntohs(fm.match.wc.masks.ivlan.vlan_tci));
    free(string_s);
    ...
    if (error) {
        ...
        printf("%04"PRIx16"/%04"PRIx16"\n",
               ntohs(nxm_match.flow.ivlan.vlan_tci),
               ntohs(nxm_match.wc.masks.ivlan.vlan_tci));
    }
    free(nxm_s);
    ofpbuf_uninit(&nxm);
    ...
    if (error) {
        ...
        uint16_t vid = ntohs(nxm_match.flow.ivlan.vlan_tci) &
            (VLAN_VID_MASK | VLAN_CFI);
        uint16_t mask = ntohs(nxm_match.wc.masks.ivlan.vlan_tci) &
            (VLAN_VID_MASK | VLAN_CFI);

        printf("%04"PRIx16"/%04"PRIx16",", vid, mask);
        if (vid && vlan_tci_to_pcp(nxm_match.wc.masks.ivlan.vlan_tci)) {
            printf("%02"PRIx8"\n", vlan_tci_to_pcp(nxm_match.flow.ivlan.vlan_tci));
        } else {
            printf("--\n");
        }
    }
    ...
    printf("OF1.0: %04"PRIx16"/%d,%02"PRIx8"/%d -> %04"PRIx16"/%04"PRIx16"\n",
           ...
           ntohs(of10_match.flow.ivlan.vlan_tci),
           ntohs(of10_match.wc.masks.ivlan.vlan_tci));
    ...
    printf("OF1.1: %04"PRIx16"/%d,%02"PRIx8"/%d -> %04"PRIx16"/%04"PRIx16"\n",
           ...
           ntohs(of11_match.flow.ivlan.vlan_tci),
           ntohs(of11_match.wc.masks.ivlan.vlan_tci));
}
