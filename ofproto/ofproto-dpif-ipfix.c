                       ...
                       const struct flow_tnl *tunnel_key)
{
     ...
     * sampled packet. */
    l2 = (flow->ivlan.vlan_tci == 0) ? IPFIX_PROTO_L2_ETH : IPFIX_PROTO_L2_VLAN;
    ...
    if (l2 == IPFIX_PROTO_L2_VLAN) {
        ...
        uint16_t vlan_id = vlan_tci_to_vid(flow->ivlan.vlan_tci);
        uint8_t priority = vlan_tci_to_pcp(flow->ivlan.vlan_tci);
